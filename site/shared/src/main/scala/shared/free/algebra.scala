...
import shared.{ EvaluationSuccess, ExerciseEvaluation, Library, Section }
...
final case class Evaluate(exerciseEvaluation: ExerciseEvaluation) extends ExerciseOp[Throwable Xor EvaluationSuccess]
...
class ExerciseOps[F[_]](implicit I: Inject[ExerciseOp, F]) {
  ...
  def evaluate(evaluation: ExerciseEvaluation): Free[F, Throwable Xor EvaluationSuccess] =
